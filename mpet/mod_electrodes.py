...
class Mod2var(dae.daeModel):
    def __init__(self, config, trode, vInd, pInd, Name, Parent=None, Description=""):
        ...
        # Domain
        self.Dmn = dae.daeDomain(
            "discretizationDomain", self, dae.unit(), "discretization domain"
        )
        ...
        self.cbar = dae.daeVariable(
            "cbar", mole_frac_t, self, "Average concentration in active particle"
        )
        ...
        if self.get_trode_param("type") not in ["ACR2"]:
            ...
            self.Rxn1 = dae.daeVariable(
                "Rxn1", dae.no_t, self, "Rate of reaction 1", [self.Dmn]
            )
            self.Rxn2 = dae.daeVariable(
                "Rxn2", dae.no_t, self, "Rate of reaction 2", [self.Dmn]
            )

        # Get reaction rate function
        rxnType = config[trode, "rxnType"]
        self.calc_rxn_rate = utils.import_function(
            config[trode, "rxnType_filename"],
            rxnType,
        ...
        self.portInBulk = ports.portFromBulk(
            "portInBulk", dae.eInletPort, self, "Inlet port from e- conducting phase"
        )
    ...
    def DeclareEquations(self):
        ...
        if self.get_trode_param("noise"):
            ...
            tvec = np.linspace(0.0, 1.05 * self.config["tend"], numnoise)
            noise_data1 = noise_prefac * np.random.randn(numnoise, N)
            noise_data2 = noise_prefac * np.random.randn(numnoise, N)
            self.noise1 = sintrp.interp1d(
                tvec, noise_data1, axis=0, bounds_error=False, fill_value=0.0
            )
            self.noise2 = sintrp.interp1d(
                tvec, noise_data2, axis=0, bounds_error=False, fill_value=0.0
            )
    ...
    def sld_dynamics_1D2var(self, c1, c2, muO, act_lyte, noises):
        ...
        if self.get_trode_param("type") in ["ACR2"]:
            eta1_eff = np.array(
                [
                    eta1[i] + self.Rxn1(i) * self.get_trode_param("Rfilm")
                    for i in range(N)
                ]
            )
            eta2_eff = np.array(
                [
                    eta2[i] + self.Rxn2(i) * self.get_trode_param("Rfilm")
                    for i in range(N)
                ]
            )
        else:
        ...
        if self.get_trode_param("type") in ["diffn2", "CHR2"]:
            ...
            Dfunc = utils.import_function(
                self.get_trode_param("Dfunc_filename"),
                Dfunc_name,
...
class Mod1var(dae.daeModel):
    def __init__(self, config, trode, vInd, pInd, Name, Parent=None, Description=""):
        ...
        self.c = dae.daeVariable(
            "c", mole_frac_t, self, "Concentration in active particle", [self.Dmn]
        )
        self.cbar = dae.daeVariable(
            "cbar", mole_frac_t, self, "Average concentration in active particle"
        )
        ...
        if config[trode, "type"] not in ["ACR"]:
            ...
            self.Rxn = dae.daeVariable(
                "Rxn", dae.no_t, self, "Rate of reaction", [self.Dmn]
            )
    ...
    def DeclareEquations(self):
        ...
        if self.get_trode_param("noise"):
            ...
            self.noise = sintrp.interp1d(
                tvec, noise_data, axis=0, bounds_error=False, fill_value=0.0
            )
        ...
        # adding mu_O as variable so I can look at it
        eq = self.CreateEquation("mu_O")
        eq.Residual = self.mu_O() - mu_O
    ...
    def sld_dynamics_1D1var(self, c, muO, act_lyte, noise):
        ...
        if self.get_trode_param("type") in ["ACR"]:
            eta_eff = np.array(
                [eta[i] + self.Rxn(i) * self.get_trode_param("Rfilm") for i in range(N)]
            )
        else:
            eta_eff = eta + self.Rxn() * self.get_trode_param("Rfilm")
        Rxn = self.calc_rxn_rate(
        ...
        if self.get_trode_param("type") in ["ACR"]:
            RHS = np.array(
                [self.get_trode_param("delta_L") * self.Rxn(i) for i in range(N)]
            )
        elif self.get_trode_param("type") in ["diffn", "CHR"]:
            # Positive reaction (reduction, intercalation) is negative
            # flux of Li at the surface.
...
def get_Mmat(shape, N):
    ...
    if shape == "C3":
        ...
        if shape == "sphere":
            Vp = 4.0 / 3.0 * np.pi * Rs**3
        elif shape == "cylinder":
            Vp = np.pi * Rs**2  # per unit height
        vol_vec = Vp * volfrac_vec
        M1 = sprs.diags(
            [1.0 / 8, 3.0 / 4, 1.0 / 8], [-1, 0, 1], shape=(N, N), format="csr"
        )
        M1[1, 0] = M1[-2, -1] = 1.0 / 4
...
def calc_flux_diffn(c, D, Dfunc, E_D, Flux_bc, dr, T, noise):
    ...
    if noise is None:
        Flux_vec[1:N] = (
            -D * Dfunc(c_edges) * np.exp(-E_D / T + E_D / 1) * np.diff(c) / dr
        )
    else:
        Flux_vec[1:N] = (
            -D
            * Dfunc(c_edges)
            * np.exp(-E_D / T + E_D / 1)
            * np.diff(c + noise(dae.Time().Value))
            / dr
        )
    return Flux_vec
...
def calc_flux_CHR(c, mu, D, Dfunc, E_D, Flux_bc, dr, T, noise):
    ...
    if noise is None:
        Flux_vec[1:N] = (
            -D / T * Dfunc(c_edges) * np.exp(-E_D / T + E_D / 1) * np.diff(mu) / dr
        )
    else:
        Flux_vec[1:N] = (
            -D
            ...
            * Dfunc(c_edges)
            * np.exp(-E_D / T + E_D / 1)
            * np.diff(mu + noise(dae.Time().Value))
            / dr
        )
    return Flux_vec
...
def calc_flux_CHR2(
    c1, c2, mu1_R, mu2_R, D, Dfunc, E_D, Flux1_bc, Flux2_bc, dr, T, noise1, noise2
):
    N = len(c1)
    Flux1_vec = np.empty(N + 1, dtype=object)
    Flux2_vec = np.empty(N + 1, dtype=object)
    Flux1_vec[0] = 0.0  # symmetry at r=0
    Flux2_vec[0] = 0.0  # symmetry at r=0
    ...
    if noise1 is None:
        Flux1_vec[1:N] = (
            -D / T * Dfunc(c1_edges) * np.exp(-E_D / T + E_D / 1) * np.diff(mu1_R) / dr
        )
        Flux2_vec[1:N] = (
            -D / T * Dfunc(c2_edges) * np.exp(-E_D / T + E_D / 1) * np.diff(mu2_R) / dr
        )
    else:
        Flux1_vec[1:N] = (
            -D
            ...
            * Dfunc(c1_edges)
            * np.exp(-E_D / T + E_D / 1)
            * np.diff(mu1_R + noise1(dae.Time().Value))
            / dr
        )
        Flux2_vec[1:N] = (
            -D
            ...
            * Dfunc(c2_edges)
            * np.exp(-E_D / T + E_D / 1)
            * np.diff(mu2_R + noise2(dae.Time().Value))
            / dr
        )
...
def MX(mat, objvec):
    ...
    for i in range(n):
        ...
        up = mat.indptr[i + 1]
        if up > low:
            out[i] = np.sum(mat.data[low:up] * objvec[mat.indices[low:up]])
        else:
            out[i] = 0.0
    return out
